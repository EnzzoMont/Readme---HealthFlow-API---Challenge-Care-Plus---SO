...
// Caminho do Log de Regra de Negócio
...
app.MapGet("/registrar-sono/{horas}", (int horas) =>
{
    ...
    if (horas < 0)
    {
        string logMsg = $"{DateTime.Now}: ERRO DE NEGÓCIO - Tentativa de registrar sono negativo: {horas}h.";
        try { File.AppendAllText(logPath, logMsg + Environment.NewLine); }
        catch { /* Ignora erro de disco para não travar request */ }
        
        return Results.BadRequest("Erro: Horas de sono não podem ser negativas.");
    }
...
app.Run();
3. Publicação (Build)
Geramos os binários otimizados para produção e os alocamos no diretório padrão do IIS.

PowerShell

dotnet publish -o "C:\inetpub\wwwroot\HealthFlow"
4. Configuração no IIS Manager
...
Permissão: Full Control para o grupo Todos (ou IIS_IUSRS em ambiente produtivo).

📊 Estratégia de Logs (Observabilidade)
Demonstramos dois níveis distintos de monitoramento exigidos no desafio:

1. Logs de Infraestrutura (IIS)
Monitoram a saúde do servidor web e requisições HTTP.

Localização: C:\inetpub\logs\LogFiles\W3SVC...

O que registra: Erros de conexão, rotas inexistentes (404), Erros internos (500).

Exemplo de Teste: Acessar http://localhost:8081/pagina-inexistente.

Resultado no Log: GET /pagina-inexistente ... 404

2. Logs de Aplicação (Regra de Negócio)
Monitoram o comportamento do usuário e violações de regras lógicas.

Localização: C:\HealthFlowLogs\app_log.txt

O que registra: Tentativas de burlar regras de saúde (ex: sono negativo).

Exemplo de Teste: Acessar http://localhost:8081/registrar-sono/-5.

Resultado no Browser: 400 Bad Request.

Resultado no Log (Txt): ERRO DE NEGÓCIO - Tentativa de registrar sono negativo.

✅ Como Testar
Acesse http://localhost:8081 para verificar se a API está online.
